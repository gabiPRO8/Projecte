...
personajes = [
    ...
    Jugador("Johan Cruyff", speed=9, height=85, power=4.2, colour=ORANGE, habilidad=False, recarga=10, imagen_path="imagenes/cruyff.png", habilidad_activada_time=3),
    Jugador("Casillas", speed=4.5, height=120, power=4, colour=RED, habilidad=False, recarga=5, imagen_path="imagenes/casillas.png", habilidad_activada_time=3.8)
]
...
def mostrar_mensaje_inicio(imagen_jugador1, imagen_jugador2):
    
    mensaje = "Pulsa espacio para jugar"
    color_mensaje = (255, 255, 255)
    rect_color = (0, 0, 0, 128)  # Color negro translúcido
    imagen_jugador1 = pygame.transform.scale(imagen_jugador1, (150, 150))
    imagen_jugador2 = pygame.transform.scale(imagen_jugador2, (150, 150))

    while True:
        SCREEN.blit(fondo, (0, 0))

        # Dibujar imágenes de los jugadores
        SCREEN.blit(imagen_jugador1, (WIDTH // 4 - imagen_jugador1.get_width() // 2, HEIGHT // 2 - 200))
        SCREEN.blit(imagen_jugador2, (3 * WIDTH // 4 - imagen_jugador2.get_width() // 2, HEIGHT // 2 - 200))

        # Mostrar mensaje "VS" en el medio
        mensaje_versus = "VS"
        mensaje_versus_render = LETTERING.render(mensaje_versus, True, WHITE)
        mensaje_versus_rect = mensaje_versus_render.get_rect(center=(WIDTH // 2, HEIGHT // 2 - 110))
        SCREEN.blit(mensaje_versus_render, mensaje_versus_rect)
        ...
        # Dibujar mensaje
        text = LETTERING.render(mensaje, True, color_mensaje)
...
def seleccion_personajes(personajes, sound_manager):
            ...
            if evento.type == pygame.QUIT:
                ...
                if not jugador1_seleccionado:
                    if evento.key == pygame.K_d:
                        ...
                        nombre_personaje = personajes[indice_seleccion1].nombre
                        sound_manager.play_player_select_sound(nombre_personaje)
                ...
                if not jugador2_seleccionado:
                    if evento.key == pygame.K_RIGHT:
                        ...
                        nombre_personaje = personajes[indice_seleccion2].nombre
                        sound_manager.play_player_select_sound(nombre_personaje)

        if jugador1 and jugador2:
            mostrar_mensaje_inicio(jugador1.imagen, jugador2.imagen)    
