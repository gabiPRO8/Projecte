...
class Ball:
    def __init__(self, x, y, radius, color, speed_x, speed_y, spin=False, trallazo=False):
        self.x = x
        self.y = y
        self.radius = radius
        self.color = color
    ...
    def check_collision(self, paddle1, paddle2,sound_manager=None):
        ...
        if self.x - self.radius <= paddle1.rect.right and paddle1.rect.top < self.y < paddle1.rect.bottom:
            ...
            elif self.spin or self.trallazo: #Si la pelota tiene efecto o viene trallazo la controlamos
                self.spin = False
                self.trallazo = False
                self.speed_x = abs(self.min_speed) * paddle1.jugador.power
                self.speed_y = sign(self.speed_y) *self.min_speed* paddle1.jugador.power    
            else:
                self.speed_x *= -1 * paddle1.jugador.power
        ...
        if self.x + self.radius >= paddle2.rect.left and paddle2.rect.top < self.y < paddle2.rect.bottom:
            ...
            if paddle2.jugador.nombre == "Johan Cruyff" and paddle2.jugador.habilidad == True:
                ...
                self.spin = False
                self.trallazo = False
                self.speed_x = -abs(self.min_speed) * paddle2.jugador.power
                self.speed_y = sign(self.speed_y) *self.min_speed* paddle2.jugador.power
            else:
                self.speed_x *= -1 * paddle2.jugador.power
